...
reserv_text: tuple = (
    'Выберите дату', 'Укажите количество гостей', 'Укажите ваши Имя и фамилию*\n*-необязательное поле')
...
async def send_help(msg: types.Message):
    ...
    if msg.text.lower() == 'о боте':
        ...
        await msg.answer(f"Выберите дату", reply_markup=greet_kb)
...
def check_api_reserve(multiproc_dict):
    ...
    while True:
        ...
        if resp.status_code != 200:  # если нет токена
            ...
            data_reserve = pd.read_excel('downloaded_files/save.xlsx', usecols=(1, 3, 4, 5, 7), engine="openpyxl")  # открываем свой exel
